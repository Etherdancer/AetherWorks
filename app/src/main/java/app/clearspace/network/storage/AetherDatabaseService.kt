package app.clearspace.network.storage
...
class AetherDatabaseService : Service() {
    ...
    private val binder = object : IAetherIpc.Stub() {
        override fun getIndex(): String {
            return try {
                ...
                val headers = publicContent.map { unit ->
                    app.clearspace.network.discovery.ContentHeader(
        ...
        override fun insertRelayPacket(packetJson: String) {
            try {
                val packet = Json.decodeFromString<app.clearspace.network.storage.db.entity.RelayPacket>(packetJson)
                val db = AetherDatabase.getSharedDatabase()
                kotlinx.coroutines.runBlocking {
        ...
        override fun enforceStorageQuota() {
            kotlinx.coroutines.runBlocking {
                app.clearspace.network.storage.StorageQuotaManager.enforcePublicQuota(this@AetherDatabaseService)
            }
        }
    }
